...
<img src="{{ site.baseurl }}/images/0429-017.png" style="width: 400px;"/>

## 基于深度神经网络的物体检测方法

<img src="{{ site.baseurl }}/images/0429-016.png" style="width: 400px;"/>

网络结构：骨干网（抽取特征，要提取好的特征需要特征融合）+检测端（轻量化）
...
收集数据（使用现有的公开数据集、爬虫爬数据、清洗数据）

标注数据（用算法自动标注再清洗、人工标注，https://github.com/tzutalin/labelImg） 

数据集划分（训练集、验证集、测试集，无交叉，80:5:15）

（做人工智能需要很强的算力和数据，从事这方面时需要自我评估实验室的环境可以支撑到什么程度，否则研究没有说服力）
...
半监督/弱监督，不给定框，只给出定义（是否有猫，有几只猫...）

## 评价指标

评价什么：类别、位置、【数量，即给一张图，把人数给数准（应用场景：银行某个地方不应该有人，如果有人就报警；但是这个不可以直接依据人脸识别就可以判断了吗？）】

<img src="{{ site.baseurl }}/images/0429-018.png" style="width: 400px;"/>

NP难问题：两个序列的匹配/多个检测框框住同一个物体，怎么去重？

如果以AP指标作为评价指标，那么就可以把阈值调低，那么就会有很多框，查全率高但是准确率会很低

<img src="{{ site.baseurl }}/images/0429-019.png" style="width: 400px;"/>

<img src="{{ site.baseurl }}/images/0429-020.png" style="width: 400px;"/>

实验过程中需要对评价指标有一个清晰的了解，才能知道怎么样提升性能（别人论文中的性能提升是面向指标做优化还是确实模型有了提升/参加比赛的时候针对评价指标做提高）

idea is cheap，give me the code。对于想法做消解实验，证明想法有用。

<img src="{{ site.baseurl }}/images/0429-021.png" style="width: 400px;"/>
